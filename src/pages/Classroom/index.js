...
import { getCurrentUser, openTab, download } from '@/utils/helper';
...
export default ({ history }) => {
  ...
  const onPreview = () => {
    const url = queryString.parse(history.location.search);
    reportAPI.preview({}, { classroomId: url.id }).then(res => openTab(res.data));
  };
  const onDownload = () => {
    const url = queryString.parse(history.location.search);
    reportAPI.download({}, { classroomId: url.id }).then(res => download(res.data, `${course.title}.pdf`));
  };
